...
'''
#Testing to see if our sheep have acces to the locations of other sheep
print("Our first sheep is at", agents[0].x, agents[0].y, ", some other sheep they knows are at:")
...
#This makes the model run until the zombies have wiped out all of the sheep, the 
#zombies have all died,or the desired number of iterations has been reached.  

def update(frame_number):
    
#Plots our agents and their environment   
    fig.clear()  
    plt.imshow(environment)
...
#This allows the model to continue after all the mines have detonated    
...
#this loop moves our sheep and makes them eat and share food if they have neighbors
    for agent in agents:  
        agent.move()
        agent.eat()
        agent.share_with_neighbours(neighbourhood)

#this loop moves our zombies    
...
#Adds a new zombie in place of the target's location and then kills the target
        for target in target_agents:
            zombsheep.append(agentframework_zombies.Zombiesheep\
                             (environment,zombsheep, agents, [target.y, target.x]))
            agents.remove(target)


    if len(holylandmines) == 0:
        pass
#Checks if there are zombies within the range of the holy landmines if there is
...
#plots our sheep in white and our zombies in red and our landmines in gold
    ...
    #print(frame_number) #testing to make sure the model has stopped at the right point
   
#Prints an update on how the sheep vs zombie battle is going
   
    print("There are", str(len(agents)), "sheep, ", str(len(zombsheep)), \
          "zombie sheep, and", str(len(holylandmines)), "landmines remaining.") 

#prints a victory message for the zombies if they manage to convert all the sheep
    if len(agents) == 0:
        print("Braiiiiins! Zombies win!")
        
#prints a victory message for the sheep if they 
#manage to survive until the end or until all zombies die
